...
public class Meteor extends Enemy {
    ...
    public Meteor(float targetX, float targetY, GameMap gameMap, Rectangle roomBounds) {
        // Метеорит має 1 HP, високу швидкість і 2 шкоди
        super(Assets.meteorTexture, // Використовуємо спеціальну текстуру для метеора
              targetX, 
              targetY + 500, // Починаємо високо над цільовою позицією
              48, 
              48, 
              FALL_SPEED, 
              1, 
              2, 
              new MeteorAI(roomBounds), 
              gameMap);
        ...
        this.warningTexture = Assets.meteorWarningTexture; // Використовуємо спеціальну текстуру для попередження
        this.fireTexture = Assets.meteorFireTexture; // Використовуємо спеціальну текстуру для вогню
        ...
        if (!hasLanded) {
            ...
            if (hero.getBoundingRectangle().overlaps(getBoundingRectangle())) {
                ...
                if (MathUtils.random(0, 100) < 2) { // 2% шанс кожен кадр (приблизно раз на секунду)
                    hero.takeDamage(1);
                    hero.addEffect(new Poison(3.0f, 1, 1.0f, Assets.poisonEffectTexture));
                }
            }
    ...
    private void explode(Hero hero) {
        ...
        if (distToHero < explosionRadius) {
            ...
            hero.addEffect(new Poison(5.0f, 1, 1.0f, Assets.poisonEffectTexture));
        }
    }
